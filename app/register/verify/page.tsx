...
export default function RegisterVerifyPage() {
  ...
  const pending = typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('pending_register') || 'null') : null;
  ...
  async function resend() {
    ...
    await fetch('/api/send-otp', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ email, purpose: 'register'})});
    setTimeLeft(60);
    alert('OTP resent');
  }
  ...
  return (
    <div className="auth-container">
      ...
      <div className="flex gap-2">
        <button onClick={verify} className="bg-green-600">Submit OTP</button>
        <button onClick={resend} className="border" disabled={timeLeft>0}>
