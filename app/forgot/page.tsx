...
export default function ForgotPage() {
  ...
  async function onSubmit(v:{email:string}) {
    await fetch('/api/send-otp', { method: 'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ email: v.email, purpose: 'reset' })});
    alert('OTP sent');
  ...
  return (
    <div className="auth-container">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
        ...
        <button className="w-full bg-accent text-white p-2 rounded">Send OTP</button>
      </form>
    </div>
  );
}
