...
export default function InvoiceForm() {
  ...
  function onProductSelect(idx: number, productId: string) {
    ...
    const discounted = Number(p.price) * (1 - Number(p.offer_percent) / 100);
    const finalPrice = discounted + discounted * Number(p.gst_percent) / 100;
    ...
    setRow(idx, {
      ...
      offer_percent: Number(p.offer_percent),
      specs: p.specs,
      finalPrice
    });
  }
  ...
  function computeTotals() {
    ...
    const gst_total = items.reduce(
      (s, it) =>
        s + (it.finalPrice - it.price) * (it.qty || 1),
      0
    );

    const totalBeforeCoupon = items.reduce(
      (s, it) => s + it.finalPrice * (it.qty || 1),
      0
    );
  ...
  async function applyCoupon() {
    ...
    } else if (data.discount_percent && Number(data.discount_percent) > 0) {
      discount = totals.total * Number(data.discount_percent) / 100;
    }
  ...
  async function generateBillAndSave() {
    ...
    for (const it of items) {
      ...
      const newQty =
        (Number(prod.qty_available) || 0) - (it.qty || 1);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6 bg-gradient-to-br from-gray-100 to-gray-200 rounded-xl shadow-xl">
      ...
      <div className="bg-white/70 backdrop-blur-xl shadow-2xl rounded-2xl p-6 border border-white/40">
        <div className="flex justify-between items-center mb-6">
          <h3 className="font-bold text-3xl text-gray-900 tracking-wide">
            ✨ Invoice Generator
          </h3>
          <button
            onClick={logout}
            className="px-4 py-2 bg-red-600 text-white rounded-xl shadow hover:bg-red-700 transition-all"
          >
            Logout
          </button>
        </div>
